-- FASE BACKOFFICE RECARGA MANUAL
-- Secure admin RPC for commercial manual recharges.

create or replace function public.admin_create_manual_recharge(
  p_user_id uuid,
...
begin
  ...
  if p_sms_amount is null or p_sms_amount <= 0 then
    raise exception 'INVALID_SMS_AMOUNT';
  end if;
  ...
  if v_previous_available < p_sms_amount then
    raise exception 'INSUFFICIENT_INVENTORY';
  end if;

  v_next_available := v_previous_available - p_sms_amount;
  ...
  if v_has_quantity_column and v_has_admin_notes_column then
    execute '
      ...
      values ($1, null, $2, $2, $3, $4, ''approved'', $5, $6, now(), $7)
      returning id
    '
      into v_recharge_id
      using p_user_id, p_sms_amount, p_amount, v_payment_method, v_operation_code, v_admin_id, v_notes;
    ...
    execute '
      ...
      values ($1, null, $2, $2, $3, $4, ''approved'', $5, $6, now())
      returning id
    '
      into v_recharge_id
      using p_user_id, p_sms_amount, p_amount, v_payment_method, v_operation_code, v_admin_id;
    ...
    execute '
      ...
      values ($1, null, $2, $3, $4, ''approved'', $5, $6, now(), $7)
      returning id
    '
      into v_recharge_id
      using p_user_id, p_sms_amount, p_amount, v_payment_method, v_operation_code, v_admin_id, v_notes;
    ...
    execute '
      ...
      values ($1, null, $2, $3, $4, ''approved'', $5, $6, now())
      returning id
    '
      into v_recharge_id
      using p_user_id, p_sms_amount, p_amount, v_payment_method, v_operation_code, v_admin_id;
  ...
  v_previous_credits := v_profile.credits;
  ...
  update public.sms_inventory
     set available_sms = v_next_available,
         sold_sms = sold_sms + p_sms_amount,
         updated_at = now()
  ...
  if to_regclass('public.inventory_transactions') is not null then
    ...
    if v_has_inventory_recharge_columns then
      execute '
        ...
        values ($1, $2, ''sale'', $3)
      '
        using v_recharge_id, p_sms_amount, coalesce(v_notes, 'Manual recharge');
      ...
      '
        using -p_sms_amount, v_previous_available, v_next_available, v_recharge_id, coalesce(v_notes, 'Manual recharge'), v_admin_id;
    end if;
  end if;
  ...
  if to_regclass('public.profile_audit_logs') is not null then
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'total_spent', v_profile.total_spent + p_amount,
...
revoke all on function public.admin_create_manual_recharge(uuid, integer, numeric, text, text, text) from public, anon;
grant execute on function public.admin_create_manual_recharge(uuid, integer, numeric, text, text, text) to authenticated;

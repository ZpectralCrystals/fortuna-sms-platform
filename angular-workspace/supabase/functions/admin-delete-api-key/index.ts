...
import { deleteApiKey } from "../_shared/sms-provider-current.ts";
...
Deno.serve(async (req: Request) => {
  ...
  if (!nombre && !id) {
    return jsonResponse({ success: false, error: "Nombre o ID de API Key requerido." }, 422);
  }

  const provider = await deleteApiKey(ruc, { nombre: nombre || undefined, id: id || undefined });
  if (!provider.success) {
